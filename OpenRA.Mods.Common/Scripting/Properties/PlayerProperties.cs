...
namespace OpenRA.Mods.Common.Scripting
{
	...
	public class PlayerProperties : ScriptPlayerProperties
	{
		...
		{
			return Player.World.Actors.Where(actor => actor.Owner == Player && !actor.IsDead && actor.IsInAnyWorld).ToArray();
		}
		...
		public Actor[] GetGroundAttackers()
		{
			return Player.World.ActorsHavingTrait<AttackBase>()
				.Where(a => a.Owner == Player && !a.IsDead && a.IsInAnyWorld && a.Info.HasTraitInfo<MobileInfo>())
				.ToArray();
		}
		...
		public Actor[] GetActorsByType(string type)
		{
			...
			result.AddRange(Player.World.Actors
				.Where(actor => actor.Owner == Player && !actor.IsDead && actor.IsInAnyWorld && actor.Info.Name == ai.Name));
		...
		public Actor[] GetActorsByTypes(string[] types)
		{
			...
			result.AddRange(Player.World.Actors
				.Where(actor => actor.Owner == Player && !actor.IsDead && actor.IsInAnyWorld && types.Contains(actor.Info.Name)));
