...
namespace OpenRA.Mods.Common.Activities
{
	public class HarvesterDockSequence : Activity
	{
		...
		public override bool Tick(Actor self)
		{
			switch (dockingState)
			{
				...
				case DockingState.Drag:
					if (IsCanceling || !RefineryActor.IsInFrontendWorld || RefineryActor.IsDead || Harv.IsTraitDisabled)
						return true;
				...
				case DockingState.Loop:
					if (IsCanceling || !RefineryActor.IsInFrontendWorld || RefineryActor.IsDead || Harv.IsTraitDisabled || Harv.TickUnload(self, RefineryActor))
						dockingState = DockingState.Undock;
		...
		{
			if (RefineryActor.IsInFrontendWorld && !RefineryActor.IsDead && DockHostSpriteOverlay != null && !DockHostSpriteOverlay.Visible)
			{
				dockingState = DockingState.Wait;
				DockHostSpriteOverlay.Visible = true;
		...
		void NotifyUndocked(Actor self)
		{
			...
			if (RefineryActor.IsInFrontendWorld && !RefineryActor.IsDead)
				foreach (var nd in notifyDockHosts)
